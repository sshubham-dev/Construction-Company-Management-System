...
const getPaymentSchedules = async (req, res) => {
  try {
    ...
    if (paymentschedules.length === 0)
      return res.status(404).json({ error: "No Payment Schedules Found" });
...
const createPaymentSchedule = async (req, res) => {
  try {
    const user = req.user;
    const { site, date, paymentDetails } = req.body;
    ...
    if (!clientPaymentSchedule)
      return res
        .status(401)
        .json({ error: "Payment Schedule is not saved", error });
    ...
    return res
      .status(201)
      .json({
        message: "Payment Schedule Created Successfuly",
...
const savePaymentSchedule = async (req, res) => {
  try {
    ...
    const paymentSchedule = await PaymentSchedule.findById(id)
      ...
      .exec();
    if (!paymentSchedule)
      return res.status(404).json({ message: "No paymentSchedule Found" });
    ...
    if (paymentSchedule.createdBy.toString() === user?._id.toString()) {
      if (paymentSchedule.adminApprove === "Approved") {
        ...
        return res
          .status(201)
          .json({ message: "paymentSchedule Saved Successfuly" });
      } else {
        console.log("paymentSchedule is Not Approved By Every One");
        return res
          .status(400)
          .json({ message: "paymentSchedule is Not Approved By Every One" });
      }
    } else {
...
const updatePaymentSchedule = async (req, res) => {
  try {
    ...
    const updatedPaymentSchedule = await existingPaymentSchedule.save({
    ...
    if (!updatedPaymentSchedule) {
      return res
        .status(500)
        .json({ error: "Failed to update Payment Schedule" });
    }
...
const getPaymentDetails = async (req, res) => {
  try {
    ...
    if (!paymentschedule && paymentschedule?.paymentDetails.length === 0)
      return res.status(404).json({ error: "Payment Schedule Not Found" });
...
const updatePaymentDetails = async (req, res) => {
  try {
    ...
    const { workDescription, amount, paymentDate, status, paid, due } =
      req.body;
    const paymentSchedule = await PaymentSchedule.findById(_id);
    if (!paymentSchedule) {
      return res.status(404).json({ error: "Payment Schedule not found" });
