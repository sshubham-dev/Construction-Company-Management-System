...
const getAllRejects = async (req, res) => {
  try {
    ...
    const reject = await Rejected.find()
      ...
      .exec();
    if (reject.length === 0)
      return res.status(400).json({ message: "No Reject Avaliable" });
    // console.log(pendingApproval)
...
const getAllApproved = async (req, res) => {
  try {
    ...
    if (approved.length === 0)
      return res.status(400).json({ message: "No Approved Item Avaliable" });
    // console.log(pendingApproval)
      ...
      case "Ceo":
        switch (approval.approvalOf) {
          ...
          case "Work Order":
            ...
            (approval.isApproved = true), await approval.save();
            (workOrder.adminApprove = "Approved"), await workOrder.save();
          ...
          case "Extra Work":
            ...
            (approval.isApproved = true), await approval.save();
            (extraWork.adminApprove = "Approved"), await extraWork.save();
            // console.log(workOrder)
          ...
          case "Expenses":
            ...
            expenses.user?.id.message.push(
              "Expenses has been Approved By Parveen Sir"
            );
            res
              .status(201)
              .json({ message: "Expenses has been Approved By Parveen Sir" });
            break;
        ...
        switch (approval.approvalOf) {
          ...
          case "Work Order":
            const workOrder = await WorkOrder.findById(approval?.data._id)
              ...
              .exec();
            (approval.isApproved = true), await approval.save();
            (workOrder.accountheadApprove = "Approved"), await workOrder.save();
            console.log(workOrder);
            saveApproved(approval, user?._id, "Work Order");
            await Approval.findByIdAndDelete(approval?._id);
          ...
          case "Extra Work":
            ...
            console.log("extraWork.createdBy:", extraWork.createdBy);
            (approval.isApproved = true), await approval.save();
            (extraWork.accountheadApprove = "Approved"), await extraWork.save();
            // console.log(workOrder)
            saveApproved(approval, user?._id, "Extra Work");
            await Approval.findByIdAndDelete(approval?._id);
            // workOrder.createdBy?.message.push('Work Order has been Approved By Parveen Sir');
      ...
      case "Site Incharge":
        switch (approval.approvalOf) {
          case "Bill":
            ...
            res
              .status(201)
              .json({ message: "Bill has been Approved By Incharge" });
          ...
          case "Work Order":
            const workOrder = await WorkOrder.findById(approval?.data._id)
              ...
              .exec();
            (approval.isApproved = true), await approval.save();
            (workOrder.inchargeApprove = "Approved"), await workOrder.save();
            console.log(workOrder);
            saveApproved(approval, user?._id, "Work Order");
            await Approval.findByIdAndDelete(approval?._id);
            workOrder.createdBy?.message.push(
          ...
          case "Purchase Request":
            ...
            res.status(201).json({
              message: "Purchase Request has been Approved By Incharge",
            });
            break;
      ...
      case "Quality Engineer":
        switch (approval.approvalOf) {
          case "Bill":
            ...
            res
              .status(201)
              .json({ message: "Bill has been Approved By Quality" });
            // console.log('bill:', bill)
            break;
...
const sendApproveByStoreIncharge = async (data, approvalof, by) => {
  try {
    const existingAccountant = await User.findOne()
      .where("department")
      .equals("Accountant")
