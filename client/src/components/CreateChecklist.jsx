...
const CreateChecklist = ({ onClose, isEdit }) => {
  ...
  useEffect(() => {
    const fetchQualitySchedule = async () => {
      try {
        const { data } = await axios.get("/api/v1/quality-schedule");
        if (!formData.site) return;

        // Filter schedules for the selected site
        const filteredSchedules = data.filter(
          (q) => q?.site?.id?._id === formData.site
        );

        // Combine all workDetails from all filtered schedules
        const allPendingWorks = filteredSchedules.flatMap(
          (schedule) =>
            schedule.workDetails?.filter(
              (work) => work.status?.toLowerCase() === "pending"
            ) || []
        );

        setWorkDetails(allPendingWorks);
        console.log("Filtered Quality Schedules:", filteredSchedules);
        console.log("Pending Work Details:", allPendingWorks);
      } catch (error) {
        console.error("Error fetching quality schedule:", error);
      }
    };

    fetchQualitySchedule();
  }, [formData.site]);
  ...
  const handleSubmit = () => {
    ...
    if (!isEdit) {
      const selectedChecklist = checkListWork.find(
        (item) => item.title === formData.name
      );
      ...
      <section className="container mx-auto mt-4 mb-16">
        <div className=" max-w-3xl mx-auto">
          {!showChecklist ? (
            <div>
              <div className="mb-4">
                <label htmlFor="site" className="block mb-1">
                  Site
                </label>
                <select
                  name="site"
              ...
              <div className="mb-4">
                <label htmlFor="date" className="block mb-1">
                  Date: {moment(formData.date).format("DD MMMM YYYY")}
                </label>
                <input
                  id="date"
                  type="date"
              ...
              <div className="mb-4">
                <label htmlFor="checkFor" className="block mb-1">
                  Checklist For
                </label>
                <select
                  name="checkFor"
                ...
                >
                  ...
                  {workDetails?.map((work, index) => (
                    <option key={index} value={work.work}>
                      {work.work}
                    </option>
                  ))}
                </select>
              ...
              <div className="mb-4">
                <label htmlFor="checklistname" className="block mb-1">
                  Checklist Name
                </label>
                <select
                  name="name"
                ...
                >
                  ...
                  {checkListWork.map((work) => (
                    <option key={work._id} value={work.title}>
                      {work.title}
                    </option>
                  ))}
                </select>
                ...
                <div className="mt-8">
                  ...
                  <div className="space-y-4 ">
                    {formData.rating.map((rating) => (
                      <div
                        key={rating.category}
                        className="flex items-center justify-between flex-col md:flex-row lg:flex-row gap-8 "
                      >
