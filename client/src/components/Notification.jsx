...
const Notification = () => {
  ...
  const displayed = (
    activeTab === "seen" ? seenNotifications : unseenNotifications
  )
      ...
      {isOpen && (
        <div className="fixed sm:absolute right-3 sm:right-0 top-20 sm:top-auto mt-1 md:mt-6 lg:mt-6 xl:mt-6 w-11/12 sm:w-96 bg-white rounded-xl shadow-xl z-50 p-4">
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            {/* Close & Mark All as Seen */}
            <div className="flex items-center gap-3">
              {activeTab === "unseen" && unseenNotifications.length > 0 && (
                <button
          ...
          <div className="max-h-96 lg:max-h-80 overflow-y-auto space-y-2 scrollbar-hide py-4">
            {displayed.length > 0 ? (
              displayed.map((msg, index) => (
                ...
                >
                  <Link to={msg.link ? msg.link : ""} className="font-semibold">
                    {msg.title}
                  </Link>
                  <div>{msg.message}</div>
