...
const CreateLeave = ({ onClose, editId }) => {
  ...
  const fetchLeave = async (id) => {
    try {
      ...
      setLeave({
        reportingDate: leaveResponse.data?.reportingDate,
        from: leaveResponse.data?.from,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log(leave);
      if (editId !== undefined) {
        const response = await axios.put(`/api/v1/leave/${editId}`, leave);
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="mb-4">
          <label
            ...
            className="block text-sm font-medium text-gray-700"
          >
            Reporting Date*: {leave.reportingDate}
          </label>
          <input
            type="date"
            id="reportingDate"
